...
# For debugging
current_method = "Gradient Boosting"
...
# Train
model = MultiOutputRegressor(GradientBoostingRegressor(n_estimators = 100, max_depth =3, learning_rate = 0.1, random_state = 42))
...
if "GB Predicted" in book.sheetnames:
    del book["GB Predicted"]
    book.save(test_path)
with pd.ExcelWriter(test_path, engine = "openpyxl", mode = "a") as writer:
    pred_df.to_excel(writer, sheet_name = "GB Predicted", index = False)
