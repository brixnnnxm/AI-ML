...
# Train
model = MultiOutputRegressor(RandomForestRegressor(n_estimators = 200, max_depth =10, min_samples_split = 5, random_state = 42, n_jobs = -1))
...
if "RF Predicted" in book.sheetnames:
    del book["RF Predicted"]
    book.save(test_path)
with pd.ExcelWriter(test_path, engine = "openpyxl", mode = "a") as writer:
    pred_df.to_excel(writer, sheet_name = "RF Predicted", index = False)
